...
alter table "public"."dx_intl_variants" add constraint "dx_intl_variants_version_check1" CHECK (version >= 0 AND version <= 26);

-- Convert dx_intl_constants from a materialized view to a single-row table so
-- future version bumps are a plain UPDATE instead of recreating every dependent
-- view. dx_intl_rating_target_stats depends on it, so it is dropped first and
-- recreated unchanged afterwards.
DROP MATERIALIZED VIEW dx_intl_rating_target_stats;
DROP MATERIALIZED VIEW dx_intl_constants;

-- Keep this a TABLE, not a materialized view. A matview bakes the values into its
-- defining query, so changing them means dropping and recreating every dependent
-- view, which also drops the Hasura tracking and permissions on
-- dx_intl_rating_target_stats (requiring a `hasura metadata apply` to restore).
-- As a table, future version bumps are just:
--
--   UPDATE dx_intl_constants
--   SET active_since = TIMESTAMPTZ '<release day> 04:00:00+09',
--       current_version_threshold = <latest version - 1>;
--
-- New values are picked up on the next REFRESH MATERIALIZED VIEW CONCURRENTLY
-- dx_intl_rating_target_stats (see hooks/src/refresh.ts). dx_intl_constants
-- itself is never refreshed.
CREATE TABLE dx_intl_constants (
  -- CHECK (id) with a boolean PK allows only one row
  id                        BOOLEAN PRIMARY KEY DEFAULT true CHECK (id),
  -- Only players updated after this are counted; the new version's release time
  active_since              TIMESTAMPTZ NOT NULL,
  -- Variants with version >= this count as "current version" for the
  -- best-15 / best-35 split; one less than the latest version
  current_version_threshold INTEGER NOT NULL
);

INSERT INTO dx_intl_constants (active_since, current_version_threshold)
VALUES (TIMESTAMPTZ '2026-07-23 04:00:00+09', 25);
