...
if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
	...
	cd /work && pg_upgrade \
	--old-datadir /var/lib/postgresql/16/data/pgdata \
	--new-datadir /var/lib/postgresql/18/data/pgdata \
	--old-bindir /usr/lib/postgresql/16/bin \
	--new-bindir /usr/lib/postgresql/18/bin

	cat /work/update_extensions.sql
    pg_checksums --enable --pgdata=/var/lib/postgresql/18/data/pgdata
	pg_setup_hba_conf "$@"
